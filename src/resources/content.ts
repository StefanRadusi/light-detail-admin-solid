...
export const getContentTree = query(async () => {
  ...
  try {
    ...
    console.error("[Content] Failed to fetch content tree, using defaults:", error);
  }
  return [];
}, "contentTree");
...
export const getContentSection = query(async (key: string) => {
  ...
  try {
    ...
    console.warn(`[Content] Failed to fetch section "${key}", components will use fallback values`);
  }
  return null;
}, "contentSection");
